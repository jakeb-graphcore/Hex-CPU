...
logic [p_data_width-1:0]    oreg        = '0; //full operand register
logic [op_width-1:0] instruction = '0; // instruction code
logic [p_data_width-1:0]    areg        = '0; // left side operand
logic [p_data_width-1:0]    breg        = '0; // right side operand
...
always@(i_instr_rd_data) begin
  ...
  unique case (instruction)
    ...
    BR: begin
        if (oreg == 254) begin
            $display("\nareg = %d\n", areg);
            //somehow kill the program without error
            $finish;
        end
        pc = pc + oreg;
    end
